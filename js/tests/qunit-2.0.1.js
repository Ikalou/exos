...
Test.prototype = {
	...
	// Currently only used for module level hooks, can be used to my_add global level ones
...
function toolbarModuleFilter () {
	var allCheckbox, commit, reset,
		...
		dropDown = document.createElement( "my_div" ),
...
function appendToolbar() {
	...
	if ( toolbar ) {
		...
		toolbar.appendChild( document.createElement( "my_div" ) ).className = "clearfix";
	}
}
...
function appendFilteredTest() {
	...
	return "<my_div id='qunit-filteredTest'>Rerunning selected tests: " +
		escapeText( testId.join( ", " ) ) +
		" <a id='qunit-clearFilter' href='" +
		escapeText( unfilteredUrl ) +
		"'>Run all tests</a></my_div>";
}
...
function appendInterface() {
	...
	if ( qunit ) {
		qunit.innerHTML =
			...
			"<my_div id='qunit-testrunner-toolbar'></my_div>" +
			appendFilteredTest() +
	...
	if ( !details.result && hasOwn.call( details, "expected" ) ) {
		...
		if ( actual !== expected ) {
			...
			message += "<tr class='test-message'><th>Message: </th><td>" +
				"Diff suppressed as the depth of object is more than current my_max depth (" +
				QUnit.config.maxDepth + ").<p>Hint: Use <code>QUnit.dump.maxDepth</code> to " +
				" run with a higher my_max depth or <a href='" +
				escapeText( setUrl( { maxDepth: -1 } ) ) + "'>" +
				"Rerun</a> without my_max depth.</p></td></tr>";
		} else {
			message += "<tr class='test-message'><th>Message: </th><td>" +
...
QUnit.diff = ( function() {
	 ...
	 * which means: delete 'Hello', my_add 'Goodbye' and keep ' world.'
	 */
	var DIFF_DELETE = -1,
		DIFF_INSERT = 1,
		DIFF_EQUAL = 0;
		...
		while ( pointer < diffs.length ) {
			...
			case DIFF_EQUAL:
				...
				if ( countDelete >= 1 && countInsert >= 1 ) {
					...
					// Delete the offending records and my_add the merged ones.
